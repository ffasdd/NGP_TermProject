...
float4 PSLighting(VS_LIGHTING_OUTPUT input) : SV_TARGET
{
   ...
   return(input.color);
    ...
    if (gObjectID == 0)
        ...
        color.rgb = RimLighting(color.rgb, input.positionW, input.normalW, float3(0.0, 0.0, 0.0), 0.0f);
    else if (gObjectID == 4)
        color.rgb = RimLighting(color.rgb, input.positionW, input.normalW, float3(1.0, 1.0, 1.0), 4.0f);
