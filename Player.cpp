...
CMyTankPlayer::~CMyTankPlayer()
{
	for (int i = 0; i < BULLETS; i++) if (m_ppBullets[i]) delete m_ppBullets[i];
}

void CMyTankPlayer::OnInitialize()
{
//	m_pMainRotorFrame = FindFrame("rotor");
//	m_pTailRotorFrame = FindFrame("black_m_7");

	m_ptankbotFrame = FindFrame("tankbot");
	m_ptanktopFrame = FindFrame("tanktop");
}

void CMyTankPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
...
void CMyTankPlayer::Animate(float fTimeElapsed, XMFLOAT4X4 *pxmf4x4Parent)
{
	if (m_ptankbotFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationY(XMConvertToRadians(360.0f) );
		m_ptankbotFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_ptankbotFrame->m_xmf4x4Transform);
	}
	if (m_ptanktopFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f) );
		m_ptanktopFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_ptanktopFrame->m_xmf4x4Transform);
	}         
...
void CMyTankPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{

	for (int i = 0; i < BULLETS; i++)if(m_ppBullets[i]->m_bActive)if(m_ppBullets[i]) m_ppBullets[i]->Render(pd3dCommandList,pCamera);

	CPlayer::Render(pd3dCommandList,pCamera);
}

void CMyTankPlayer::FireBullet()
{
	...
	XMFLOAT3 NormalizelLook = Vector3::Normalize(Vector3::Add(PlayerLook, CameraLook));

	if (pBulletObject)
	{
		XMFLOAT3 xmf3Position = m_ptanktopFrame->GetPosition();
		XMFLOAT3 xmf3Direction = m_ptanktopFrame->GetLook();
		pBulletObject->m_xmf4x4Transform = m_xmf4x4World;
		XMFLOAT3 xmf3FirePosition = Vector3::Add(xmf3Position, Vector3::ScalarProduct(xmf3Direction, 100.0f, true));
		xmf3FirePosition.y += 10.0f;
...
void CMyTankPlayer::OnPrepareRender()
{
	CPlayer::OnPrepareRender();
}

CCamera *CMyTankPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
