...
void CGameFramework::Gameover() {
	...
	D2D1_RECT_F d2dRect = D2D1::RectF(
		(float)m_nWndClientWidth / 4.0f,   // 왼쪽
		(float)m_nWndClientHeight / 4.0f,  // 위
		(float)m_nWndClientWidth * 3.0f / 4.0f,   // 오른쪽
		(float)m_nWndClientHeight * 3.0f / 4.0f  // 아래
	);
	WCHAR pstrOutputText[256];
	wcscpy_s(pstrOutputText, 256, L"게임 오버\n");
	m_pUILayer->UpdateTextOutputs(0, pstrOutputText, &d2dRect, pdwTextFormat, pd2dBrush);
}
